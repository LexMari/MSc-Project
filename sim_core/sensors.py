...
once an attack is injected. Keeping SensorReading as plain data (not tied
to vehicle physics) is what lets attacks.py manipulate readings without
...
class SensorReading:
    ...
    detected_position: tuple[float, float] | None = None    # for GPS
    signal_state: str | None = None                         # for camera reading a traffic light, e.g. "red"/"green"
    confidence: float = 1.0
    is_attacked: bool = False   # ground-truth flag for logging only
