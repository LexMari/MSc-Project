...
Both give a fatality-risk probability from impact speed. Turning that
into slight/serious/fatal happens in two steps: fatal vs not-fatal is
sampled against the cited probability using the scenario's seeded RNG
(so running the same scenario many times gives a real distribution of
outcomes, not the same answer every time), and serious vs slight for
non-fatal cases uses SERIOUS_FLOOR below, which is this project's own
threshold, not from either cited source.
"""
...
def vehicle_fatality_risk(closing_speed_ms: float) -> float:
    ...
    Fitted (least-squares logistic) to three published risk points for
    belted drivers in frontal impacts, Richards (2010) Fig 3.3: ~3% at
    30mph, ~17% at 40mph, ~60% at 50mph. Frontal rather than side impact
...
def classify_severity(fatality_risk: float, rng) -> str:
    """Samples "slight" | "serious" | "fatal" from a fatality-risk
